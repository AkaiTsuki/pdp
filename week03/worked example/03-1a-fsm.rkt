...
(define (next-state s inp)
  (cond
    ...
    [(string=? s "state3") (after-state3 inp)]
...
;; in state2, we've seen a*(b|c).  Returns a state that describes what we've seen after
;; the given input
...
;; after-state3 : Input -> State
;; returns the state that should follow state3 after the given input
;; we are in state3, so we have seen a*(b|c)d+
 (define (after-state3 inp)
   (cond
     [(string=? inp "a") "errorstate"]
     [(string=? inp "b") "errorstate"]
     [(string=? inp "c") "errorstate"]
...
;; Note 2:
;; When I ran this in class, I realized that I had gotten the
