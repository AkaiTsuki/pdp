...
#reader(lib "htdp-beginner-reader.ss" "lang")((modname two-draggable-cats) (read-case-sensitive #t) (teachpacks ()) (htdp-settings #(#t constructor repeating-decimal #f #t none #f ())))
;; two draggable cats.
...
(define (main initial-y-pos)
  (big-bang (initial-world initial-y-pos)    
...
;; produces the world that should follow the given world after a tick
...
;; produces the state of the given cat after a tick.
;; INVARIANT: the world is not paused
...
;; produces the cat that should follow one in the given position in an
;; unpaused world 
...
;; produces the world that should follow the given world
...
;; produces a world just like the given one, but with paused? toggled
;; strategy: structural decomposition on w : World
...
;; produce the cat following a button-down at the given location.
...
;; produce the cat following a drag at the given location
;; strategy: struct decomp on cat
...
;; produce the cat following a button-up at the given location
;; strategy: struct decomp on cat
...
;; produces a world with two unselected cats at the given y coordinate
