...
;; version 3: In this version, we'll make the
...
(define Container%
  (class* object% (StatefulWorldObj<%>)
    ...
    ;; to the container, that new object will be thrown away!  [This
...
;; right edges.
;; A Ball is a (new Ball% [x Number][y Number][box Box][speed Number])
(define Ball%
...
(define BallFactory%
  (class*
   ...
   ;; catch "n" events here and create a new ball. 
    ...
    ;; to the container as a functional object.
       ...
       ((define the-center (send box center))
        (define the-ball
          (new Ball% 
               ...
               [box box]
               [speed speed])))
       (send container add-object the-ball)))
...
(test-case
 ...
 (check-equal? (balldata-x (send ball-after-tick probe)) 
               (+ 100 5) 
...
(test-case
 ...
 (check-equal? 
  (balldata-speed (send ball-after-tick probe))
  -10
...
(test-case
 ...
 ;; drag the box
 (send the-box on-mouse 154 50 "drag")
 (check-equal?
...
;; this was a complicated test sequence I made up when I was trying to figure out 
;; why balls were not being added correctly.
;; I tried adding balls directly to the container, then adding them through more
;; and more layers.
(test-case 
 "check that balls are added correctly"
 ;; first create a box, a ball in that box, and a container containing 
 ...
 ;; add a ball directly to the container
 (send the-container add-object (new Ball% [x 100][y 45][box the-box][speed 5]))
 (check-equal?
  (map length (send the-container probe))
  '(2 1)
  "check adding a ball to the container")
 ;; create a factory
 (define factory1
       (new BallFactory%
            [container the-container]
            [box       the-box]))
 ;; tell the factory directly to add a ball to the container
 (send factory1 add-ball)
 ;; along around here I got tired of  writing (map (length ...)),
 ;; so I defined check-lengths
 (check-lengths the-container '(3 1) "check adding a ball through a container")
 ;; now put the factory in the container
 (send the-container add-stateful-object factory1)
 (check-lengths the-container '(3 2) "factory should show up as another stateful object")
 ;; see if factory responds to on-key "n"
 (send factory1 on-key "n")
 (check-lengths the-container '(4 2) "factory should respond to on-key 'n'")
 ;; when I made the factory functional, everything worked up to here.
 ;; now try sending the "n" to  the container instead of the factory
 (send the-container on-key "n")
 (check-lengths the-container '(5 2) "container should respond to on-key 'n'"))
