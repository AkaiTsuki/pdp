...
(define Ball%
  (class* object% (WorldObj<%>)
    ...
    (init-field [selected? false])   ; is the ball selected?
...
(test-begin
 ...
 (check-lengths the-container '(3 1))
 (send the-container add-stateful-object factory1)
 (check-lengths the-container '(3 2))
 (send factory1 on-key "n")
 (check-lengths the-container '(4 2))
 (send the-container on-key "n")
 (check-lengths the-container '(5 2)))
